...
public class Shuffle<K, V> extends Task {// implements ExceptionReporter
  ...
  private final ReduceTask reduceTask; // Used for status updates
...
//    this.metrics = new ShuffleClientMetrics(reduceId, jobConf);
  ...
  private ShuffleClientInstrumentation createShuffleClientInstrumentation() {
    return DefaultMetricsSystem.INSTANCE.register("ShuffleClientMetrics",
        "Shuffle input metrics", new ShuffleClientInstrumentation(this.jobConf,
            this.reduceTask.getJobID().toString(), this.taskStatus.getTaskID()
                .getTaskID()));
    // //may be getTaskId as last args wrong.
  }

  public class ReduceCopier implements MRConstants {

    /** Reference to the umbilical object */
    private TaskUmbilicalProtocol umbilical = Shuffle.this.umbilical;
    private final TaskReporter reporter = Shuffle.this.reporter;
    private final ReduceTask reduceTask = Shuffle.this.reduceTask;
    ...
    // * Maximum percent of shuffle execution time required to keep the reducer
    // alive.
    ...
    /**
     * The minimum percentage of maps yet to be copied, which indicates end of
     * shuffle
     */
    private static final float MIN_PENDING_MAPS_PERCENT = 0.25f;
    /**
     * Maximum no. of unique maps from which we failed to fetch map-outputs even
     * after {@link #maxFetchRetriesPerMap} retries; after this the reduce task
     * is failed.
     */
    ...
    /**
     * The map for (Hosts, List of MapIds from this Host) maintaining map output
     * locations
     */
    ...
    // {
    // return DefaultMetricsSystem.INSTANCE.register("ShuffleClientMetrics",
    // "Shuffle input metrics", new ShuffleClientInstrumentation(jobConf));
    // }
    ...
    private CompressionCodec lfsmergeCodec;
    ...
    public class MapOutputLocation {
      ...
      public MapOutputLocation(TaskAttemptID taskAttemptId, String ttHost,
          URL taskOutput) {
    ...
    public class ShuffleRamManager implements RamManager {
      ...
      public ShuffleRamManager(Configuration conf) throws IOException {
        ...
        maxSize = (int) (conf
            .getInt("mapred.job.reduce.total.mem.bytes", (int) Math.min(Runtime
                .getRuntime().maxMemory(), Integer.MAX_VALUE)) * maxInMemCopyUse);
...
//        this.reporter = reporter;
        ...
        shuffleConnectionTimeout = job.getInt(
            "mapreduce.reduce.shuffle.connect.timeout", STALLED_COPY_TIMEOUT);
        shuffleReadTimeout = job.getInt(
            "mapreduce.reduce.shuffle.read.timeout", DEFAULT_READ_TIMEOUT);
      ...
      /**
       * Loop forever and fetch map outputs as they become available. The thread
       * exits when it is interrupted by {@link ReduceTaskRunner}
       */
      @Override
      public void run() {
        while (true) {
      ...
      private long copyOutput(MapOutputLocation loc) throws IOException,
        ...
        // // change synchronized(ReduceTask.this) to this.
        // // may be bug
        synchronized (this) {
          if (copiedMapOutputs.contains(loc.getTaskId())) {
            mapOutput.discard();
       ...
       * the local fs) from the remote server. We use the file system so that we
       * generate checksum files on the data.
       * 
       * @param mapOutputLoc
      ...
      private MapOutput getMapOutput(MapOutputLocation mapOutputLoc,
        ...
        if (!mapId.equals(expectedMapId)) {
          LOG.warn("data from wrong map:" + mapId + " arrived to reduce task "
              + reduce + ", where as expected map output should be from "
              + expectedMapId);
          return null;
        }
...
//        LOG.debug("_____compressdLength is " + compressedLength);
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("header: " + mapId + ", compressed len: "
              + compressedLength + ", decompressed len: " + decompressedLength);
        }
        ...
        if (shuffleInMemory) {
          if (LOG.isDebugEnabled()) {
            LOG.debug("Shuffling " + decompressedLength + " bytes ("
                + compressedLength + " raw bytes) " + "into RAM from "
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug("Shuffling " + decompressedLength + " bytes ("
                + compressedLength + " raw bytes) " + "into Local-FS from "
                + mapOutputLoc.getTaskAttemptId());
          }
...
//        LOG.debug("____before setRequestProperty");
       ...
       * up only on the last failure. Instead of connecting with a timeout of X,
       * we try connecting with a timeout of x < X but multiple times.
       */
      ...
      private MapOutput shuffleInMemory(MapOutputLocation mapOutputLoc,
          URLConnection connection, InputStream input, int mapOutputLength,
          int compressedLength) throws IOException, InterruptedException {
        ...
        if (bytesRead != mapOutputLength) {
          ...
          throw new IOException("Incomplete map output received for "
              ...
              + mapOutputLoc.getOutputLocation() + " (" + bytesRead
              + " instead of " + mapOutputLength + ")");
        }
      ...
      private MapOutput shuffleToDisk(MapOutputLocation mapOutputLoc,
        ...
        Path localFilename = ldir.getLocalPathForWrite(filename.toUri()
            .getPath(), mapOutputLength, jobConf);

        MapOutput mapOutput = new MapOutput(mapOutputLoc.getTaskId(),
            mapOutputLoc.getTaskAttemptId(), Shuffle.this.jobConf,
    ...
    private void configureClasspath(JobConf conf) throws IOException {
      ...
      Task task = Shuffle.this.reduceTask;
      ClassLoader parent = conf.getClassLoader();
      // get the work directory which holds the elements we are dynamically
      // adding to the classpath
      // if (task.getJobFile() == null) {
      // task = Shuffle.this;
      // LOG.debug("_____Shuffle.this.reduceTask is null");
      // }
      //
      // if (task.getJobFile() == null)
      // {
      // LOG.debug("____Job file is always null");
      // }
    ...
    public ReduceCopier() throws IOException, ClassNotFoundException {
      ...
      this.combinerRunner = CombinerRunner.create(jobConf, getTaskID(),
          combineInputCounter, reporter, null);
      if (combinerRunner != null) {
        combineCollector = new CombineOutputCollector(
            reduceCombineOutputCounter, reporter, jobConf);
      }
      ...
      this.maxInMemOutputs = jobConf.getInt("mapred.inmem.merge.threshold",
          1000);
      this.maxInMemCopyPer = jobConf.getFloat(
          "mapred.job.shuffle.merge.percent", 0.66f);
      final float maxRedPer = jobConf.getFloat(
      ...
      this.reportReadErrorImmediately = jobConf.getBoolean(
    ...
    public boolean fetchOutputs() throws IOException {
      ...
      final Progress copyPhase = Shuffle.this.reduceTask.getProgress().phase();
      ...
      for (int i = 0; i < numCopiers; i++) {
        MapOutputCopier copier = new MapOutputCopier(Shuffle.this.jobConf,
            Shuffle.this.reporter, Shuffle.this.reduceTask.getJobTokenSecret());
      ...
      while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
        ...
        if (numScheduled > 0 || logNow) {
          LOG.info(reduceTask.getTaskID() + " Scheduled " + numScheduled
              + " outputs (" + penaltyBox.size() + " slow hosts and" + numDups
              + " dup hosts)");
        }
...
//          LOG.debug(reduceTask.getTaskID() + " numInFlight = " + numInFlight);
          ...
          if (cr.getSuccess()) { // a successful copy
            ...
            totalFailures++;
            Integer noFailedFetches = mapTaskToFailedFetchesMap.get(mapTaskId);
            ...
            if (noFailedFetches >= abortFailureLimit) {
              ...
              umbilical.shuffleError(getTaskID(),
                  "Exceeded the abort failure limit;" + " bailing-out.",
                  jvmContext);
            }

            checkAndInformJobTracker(noFailedFetches, mapTaskId, cr.getError()
                .equals(CopyOutputErrorType.READ_ERROR));
            ...
            if (noFailedFetches == maxFetchFailuresBeforeReporting) {
              ...
              if ((fetchFailedMaps.size() >= maxFailedUniqueFetches || fetchFailedMaps
                  .size() == (numMaps - copiedMapOutputs.size()))
                  && !reducerHealthy
                  && (!reducerProgressedEnough || reducerStalled)) {
                LOG.fatal("Shuffle failed with too many fetch failures "
                    + "and insufficient progress!" + "Killing task "
                    + getTaskID() + ".");
                umbilical.shuffleError(getTaskID(),
            ...
            currentTime = System.currentTimeMillis();
            long currentBackOff = (long) (INITIAL_PENALTY * Math.pow(
                PENALTY_GROWTH_RATE, noFailedFetches));

            penaltyBox.put(cr.getHost(), currentTime + currentBackOff);
            LOG.warn(reduceTask.getTaskID() + " adding host " + cr.getHost()
                + " to penalty box, next contact in " + (currentBackOff / 1000)
                + " seconds");
          }
    ...
    protected void checkAndInformJobTracker(int failures, TaskAttemptID mapId,
        boolean readError) {
    ...
    /**
     * Create a RawKeyValueIterator from copied map outputs. All copying threads
     * have exited, so all of the map outputs are available either in memory or
     * on disk. We also know that no merges are in progress, so synchronization
     * is more lax, here.
     * 
     * The iterator returned must satisfy the following constraints: 1. Fewer
     * than io.sort.factor files may be sources 2. No more than maxInMemReduce
     * bytes of map outputs may be resident in memory when the reduce begins
     * 
     * If we must perform an intermediate merge to satisfy (1), then we can keep
     * the excluded outputs from (2) in memory and include them in the first
     * merge pass. If not, then said outputs must be written to disk first.
     */
    ...
    private RawKeyValueIterator createKVIterator(JobConf job, FileSystem fs,
      ...
      if (mapOutputsFilesInMemory.size() > 0) {
        ...
        if (numMemDiskSegments > 0
          ...
          final Writer writer = new Writer(job, fs, outputPath, keyClass,
              valueClass, codec, null);
          try {
      ...
      LOG.info("Merging " + finalSegments.size() + " segments, " + inMemBytes
      ...
      if (0 != onDiskBytes) {
        ...
        RawKeyValueIterator diskMerge = Merger.merge(job, fs, keyClass,
            valueClass, codec, diskSegments, ioSortFactor, numInMemSegments,
        ...
        finalSegments.add(new Segment<K, V>(new RawKVIteratorReader(diskMerge,
            onDiskBytes), true));
      }

      // //Now isLocal is TRUE and is local merge.
      return Merger.merge(job, fs, keyClass, valueClass, finalSegments,
          finalSegments.size(), tmpDir, comparator, reporter, false,
    ...
    /**
     * Starts merging the local copy (on disk) of the map's output so that most
     * of the reducer's input is sorted i.e overlapping shuffle and merge
     * phases.
     */
    public class LocalFSMerger extends Thread {
      ...
      public LocalFSMerger(LocalFileSystem fs, CompressionCodec codec) {
        this.localFileSys = fs;
        // /add codec by @nourlcn
        this.codec = codec;
      ...
      public void run() {
        try {
          ...
          while (!exitLocalFSMerge) {
            ...
            approxOutputSize += ChecksumFileSystem.getChecksumLength(
                approxOutputSize, bytesPerSum);

            // 2. Start the on-disk merge process
            Path outputPath = lDirAlloc.getLocalPathForWrite(
                mapFiles.get(0).toString(), approxOutputSize, jobConf).suffix(
                ".merged");
            Writer writer = new Writer(jobConf, rfs, outputPath,
                jobConf.getMapOutputKeyClass(),
                jobConf.getMapOutputValueClass(), codec, null);
            RawKeyValueIterator iter = null;
            Path tmpDir = new Path(reduceTask.getTaskID().toString());
            try {
              iter = Merger.merge(jobConf, rfs, jobConf.getMapOutputKeyClass(),
                  jobConf.getMapOutputValueClass(), codec,
                  mapFiles.toArray(new Path[mapFiles.size()]), true,
                  ioSortFactor, tmpDir, jobConf.getOutputKeyComparator(),
            ...
            LOG.info(reduceTask.getTaskID() + " Finished merging "
                + mapFiles.size() + " map output files on disk of total-size "
                + approxOutputSize + "." + " Local output file is "
                + outputPath + " of size "
    ...
    public class InMemFSMergeThread extends Thread {
      ...
      private void doInMemMerge() throws IOException {
        ...
        try {
          ...
          rIter = Merger.merge(jobConf, rfs, (Class<K>) jobConf
              .getMapOutputKeyClass(), (Class<V>) jobConf
              .getMapOutputValueClass(), inMemorySegments, inMemorySegments
              .size(), new Path(reduceTask.getTaskID().toString()), jobConf
              .getOutputKeyComparator(), reporter, false,
              spilledRecordsCounter, null, null);
          ...
          localFileSys.delete(outputPath, true);
          throw (IOException) new IOException("Intermediate merge failed")
              .initCause(e);
        }
    ...
    public class GetMapEventsThread extends Thread {
      ...
      public void run() {
        ...
        do {
          try {
            ...
            if (LOG.isDebugEnabled()) {
              if (numNewMaps > 0) {
                LOG.debug(reduceTask.getTaskID() + ": " + "Got " + numNewMaps
                    + " new map-outputs");
              }
            }
      ...
      private int getMapCompletionEvents() throws IOException {
        ...
        MapTaskCompletionEventsUpdate update = umbilical
            .getMapCompletionEvents(reduceTask.getJobID(), fromEventId.get(),
                MAX_EVENTS_TO_FETCH, reduceTask.getTaskID(), jvmContext);
        ...
        for (TaskCompletionEvent event : events) {
          ...
          case SUCCEEDED: {
            ...
            URL mapOutputLocation = new URL(event.getTaskTrackerHttp()
                + "/mapOutput?job=" + taskId.getJobID() + "&map=" + taskId
                + "&reduce=" + getPartition());
  ...
  // final EventFetcher<K, V> eventFetcher = new EventFetcher<K, V>(reduceId,
  // umbilical, scheduler, this, jvmContext, jobConf.getInt(
  ...
  // reporter, metrics, this, reduceTask.getJobTokenSecret());
  // fetchers[i].start();
  // }
  //
  // // ----------Now shuffleToMem or shuffleToDisk finished.--------//
  ...
  public void runWithoutMerge() throws IOException, ClassNotFoundException {
    ...
    if (!fetchState) {
      ...
      throw new IOException("Task: " + this.taskStatus.getTaskID().getTaskID()
  ...
  public Path[] getMapFiles(FileSystem fs, boolean isLocal) throws IOException {
  ...
  // if (throwable == null) {
  // throwable = t;
  // String throwingThreadName = Thread.currentThread().getName();
