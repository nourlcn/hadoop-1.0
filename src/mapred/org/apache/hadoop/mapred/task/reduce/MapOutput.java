...
class MapOutput<K, V> {
  ...
  MapOutput(TaskAttemptID mapId, MergeManager<K, V> merger, long size,
      JobConf conf, LocalDirAllocator localDirAllocator, int fetcher,
      boolean primaryMapOutput, MapOutputFile mapOutputFile) throws IOException {
    ...
    outputPath = mapOutputFile.getInputFileForWrite(mapId.getTaskID(), size);
  ...
  public static class MapOutputComparator<K, V> implements
      Comparator<MapOutput<K, V>> {
