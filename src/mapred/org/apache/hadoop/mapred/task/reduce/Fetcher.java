...
class Fetcher<K, V> extends Thread {
  ...
  public Fetcher(JobConf job, TaskAttemptID reduceId,
    ...
    this.connectionTimeout = job.getInt(
        "mapreduce.reduce.shuffle.connect.timeout",
        DEFAULT_STALLED_COPY_TIMEOUT);
    this.readTimeout = job.getInt("mapreduce.reduce.shuffle.read.timeout",
  ...
  private boolean copyMapOutput(MapHost host, DataInputStream input,
    ...
    try {
      ...
      scheduler.copySucceeded(mapId, host, compressedLength, endTime
          - startTime, mapOutput);
      ...
      if (mapId == null || mapOutput == null) {
        LOG.info("fetcher#" + id + " failed to read map header" + mapId
            + " decomp: " + decompressedLength + ", " + compressedLength, ioe);
        return false;
      }

      LOG.info(
          "Failed to shuffle output of " + mapId + " from "
              + host.getHostName(), ioe);
  ...
  private void shuffleToDisk(MapHost host, MapOutput<K, V> mapOutput,
    ...
    try {
      ...
      LOG.info("Read " + (compressedLength - bytesLeft)
          + " bytes from map-output for " + mapOutput.getMapId());
    ...
    if (bytesLeft != 0) {
      throw new IOException("Incomplete map output received for "
          + mapOutput.getMapId() + " from " + host.getHostName() + " ("
