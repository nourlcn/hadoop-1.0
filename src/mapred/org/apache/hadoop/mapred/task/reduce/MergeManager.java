...
import org.apache.hadoop.mapred.ConcatenateMerger;
...
////
//import org.apache.hadoop.mapred.UnsortedKVIterator;
...
import org.apache.hadoop.mapred.task.reduce.MapOutput.MapOutputComparator;
...
public class MergeManager<K, V> {
  ...
  Set<MapOutput<K, V>> inMemoryMergedMapOutputs = new TreeSet<MapOutput<K, V>>(
      new MapOutputComparator<K, V>());
  private final IntermediateMemoryToMemoryMerger memToMemMerger;

  Set<MapOutput<K, V>> inMemoryMapOutputs = new TreeSet<MapOutput<K, V>>(
      new MapOutputComparator<K, V>());
  private final InMemoryMerger inMemoryMerger;

  Set<MapOutput<K, V>> onDiskMapOutputs = null;
  ...
  // when avoidsort = true, only one merger thread : in memory merge thread.
  ...
  public MergeManager(TaskAttemptID reduceId, JobConf jobConf,
    ...
    this.avoidsort = jobConf.getBoolean("mapred.sort.avoidance", false);

    this.onDiskMapOutputs = new TreeSet<MapOutput<K, V>>(
        new MapOutputComparator<K, V>());

    final float maxInMemCopyUse = jobConf.getFloat(
        "mapred.job.shuffle.input.buffer.percent", 0.90f);
    if (maxInMemCopyUse > 1.0 || maxInMemCopyUse < 0.0) {
      throw new IllegalArgumentException("Invalid value for "
          + "mapred.job.shuffle.input.buffer.percent" + ": " + maxInMemCopyUse);
    }

    // Allow unit tests to fix Runtime memory
    this.memoryLimit = (long) (jobConf.getLong(
        "mapred.reduce.memory.totalbytes",
    ...
    this.memToMemMergeOutputsThreshold = jobConf.getInt(
        "mapred.inmem.merge.threshold", ioSortFactor);
    this.mergeThreshold = (long) (this.memoryLimit * jobConf.getFloat(
        "mapred.job.shuffle.merge.percent", 0.90f));
    LOG.info("MergerManager: memoryLimit=" + memoryLimit + ", "
        + "maxSingleShuffleLimit=" + maxSingleShuffleLimit + ", "
        + "mergeThreshold=" + mergeThreshold + ", " + "ioSortFactor="
        + ioSortFactor + ", " + "memToMemMergeOutputsThreshold="
        + memToMemMergeOutputsThreshold);

    boolean allowMemToMemMerge = jobConf.getBoolean(
        "mapred.inmem.merge.threshold.enabled", false);
    if (allowMemToMemMerge && !avoidsort) {
  ...
  // //get location for the map output.
  public synchronized MapOutput<K, V> reserve(TaskAttemptID mapId,
      long requestedSize, int fetcher) throws IOException {
  ...
  public synchronized Segment<K, V> getMapOutput() throws IOException {
    while (inMemoryMapOutputs.size() == 0 && onDiskMapOutputs.size() == 0) {
      if (close) {
        return null;
      }
      try {
...
//          (int) ((long) mo.getMemory().length));
      ...
      MapOutput<K, V> onDisk = ((TreeSet<MapOutput<K, V>>) onDiskMapOutputs)
          .pollFirst();
      ...
      LOG.info("Feed on disk file -> size " + onDisk.getSize() + " to reduce");
  ...
  public RawKeyValueIterator close() throws Throwable {
    ...
    // return null;
    // } else {
    List<MapOutput<K, V>> memory = new ArrayList<MapOutput<K, V>>(
  ...
  private class IntermediateMemoryToMemoryMerger extends
      MergeThread<MapOutput<K, V>, K, V> {
    ...
    public void merge(List<MapOutput<K, V>> inputs) throws IOException {
      ...
      MapOutput<K, V> mergedMapOutputs = unconditionalReserve(dummyMapId,
          mergeOutputSize, false);

      Writer<K, V> writer = new InMemoryWriter<K, V>(
          mergedMapOutputs.getArrayStream());
      // if (!avoidsort) {
      // LOG.info("Initiating sorted Memory-to-Memory merge with "
      // + noInMemorySegments + " segments of total-size: "
      // + mergeOutputSize);
      //
      // RawKeyValueIterator rIter = Merger.merge(jobConf, rfs,
      // (Class<K>) jobConf.getMapOutputKeyClass(),
      // (Class<K>) jobConf.getMapOutputValueClass(), codec, inMemorySegments,
      // inMemorySegments.size(), new Path(reduceId.toString()),
      // (RawComparator<K>) jobConf.getOutputKeyComparator(), reporter,
      // null, null);
      //
      // Merger.writeFile(rIter, writer, reporter, jobConf);
      // } else {
      LOG.info("Initiating unsorted Memory-to-Memory merge with "
          + noInMemorySegments + " segments of total-size: " + mergeOutputSize);
      ConcatenateMerger.writeFile(writer, inMemorySegments, reporter, null,
          jobConf);
      // }

      writer.close();

      LOG.info(reduceId + " Memory-to-Memory merge of the "
  ...
  private class InMemoryMerger extends MergeThread<MapOutput<K, V>, K, V> {
    ...
    public void merge(List<MapOutput<K, V>> inputs) throws IOException {
      ...
      Path outputPath = mapOutputFile.getInputFileForWrite(mapTaskId,
          mergeOutputSize).suffix(Task.MERGED_OUTPUT_PREFIX);
      long uncompressedSize = -1;

      try {
        if (!avoidsort) {
          Writer<K, V> writer = new Writer<K, V>(jobConf, rfs, outputPath,
              (Class<K>) jobConf.getMapOutputKeyClass(),
              (Class<V>) jobConf.getMapOutputValueClass(), codec, null);

          LOG.info("Initiating sorted in-memory merge with "
              + noInMemorySegments + " segments...");
          RawKeyValueIterator rIter = null;
          rIter = Merger.merge(jobConf, rfs,
              (Class<K>) jobConf.getMapOutputKeyClass(),
              (Class<V>) jobConf.getMapOutputValueClass(), inMemorySegments,
              inMemorySegments.size(), new Path(reduceId.toString()),
              (RawComparator<K>) jobConf.getOutputKeyComparator(), reporter,
              false, spilledRecordsCounter, null, null);
          ...
          writer.close();
          uncompressedSize = writer.getRawLength();

          LOG.info(reduceId + " Merge of the " + noInMemorySegments
              + " files in-memory complete." + " Local file is " + outputPath
              + " of size " + localFS.getFileStatus(outputPath).getLen());
        } else {
          LOG.info("Initiating unsorted in-memory  merge with "
              + noInMemorySegments + " segments...");
          Writer<K, V> writer = new Writer<K, V>(jobConf, rfs, outputPath,
              null, null, codec, null);
          ConcatenateMerger.writeFile(writer, inMemorySegments, reporter,
              spilledRecordsCounter, jobConf);
          writer.close();
          uncompressedSize = writer.getRawLength();
        }
      } catch (IOException e) {
      ...
      // Note the output of the merge
      closeOnDiskFile(new MapOutput<K, V>(outputPath, uncompressedSize));
    }
  ...
  private class OnDiskMerger extends MergeThread<MapOutput<K, V>, K, V> {
    ...
    public void merge(List<MapOutput<K, V>> inputs) throws IOException {
      ...
      for (MapOutput<K, V> file : inputs) {
        approxOutputSize += localFS.getFileStatus(file.getOutputPath())
            .getLen();
      }
      ...
      Path outputPath = localDirAllocator.getLocalPathForWrite(
          inputs.get(0).toString(), approxOutputSize, jobConf).suffix(
      ...
      try {
        if (!avoidsort) {
          ...
          iter = Merger.merge(jobConf, rfs,
              (Class<K>) jobConf.getMapOutputKeyClass(),
              (Class<V>) jobConf.getMapOutputValueClass(), codec, tmpInputs,
              true, ioSortFactor, tmpDir,
              (RawComparator<K>) jobConf.getOutputKeyComparator(), reporter,
              spilledRecordsCounter, mergedMapOutputsCounter, null);
...
//              true, ioSortFactor, tmpDir,
//              (RawComparator<K>) jobConf.getOutputKeyComparator(), reporter,
//              spilledRecordsCounter, null, mergedMapOutputsCounter, null, null);
          ...
          for (MapOutput<K, V> file : inputs) {
            ...
            Path tmp = file.getOutputPath();
            segments.add(new Segment<K, V>(jobConf, rfs, tmp, 0, rfs
                .getFileStatus(tmp).getLen(), file.getSize(), codec, false,
                (file.toString().endsWith(Task.MERGED_OUTPUT_PREFIX) ? null
                    : mergedMapOutputsCounter)));
          }

          ConcatenateMerger.writeFile(writer, segments, reporter,
              spilledRecordsCounter, jobConf);
          writer.close();
          uncompressedSize = writer.getRawLength();
      ...
      LOG.info(reduceId + " Finished merging " + inputs.size()
  ...
  private void combineAndSpill(RawKeyValueIterator kvIter,
    ...
    try {
      CombineValuesIterator values = new CombineValuesIterator(kvIter,
          comparator, keyClass, valClass, job, Reporter.NULL, inCounter);
      while (values.more()) {
  ...
  private long createInMemorySegments(List<MapOutput<K, V>> inMemoryMapOutputs,
      List<Segment<K, V>> inMemorySegments, long leaveBytes) throws IOException {
    long totalSize = 0L;
    // We could use fullSize could come from the RamManager, but files can
    // be
...
//          mo.getMapId(), data, 0, (int) size);
  ...
  // private RawKeyValueIterator unSortedMerge(JobConf job, FileSystem fs,
  // List<MapOutput<K, V>> inMemoryMapOutputs, List<Path> onDiskMapOutputs)
  // throws IOException {
  // LOG.info("unsorted finalMerge called with " + inMemoryMapOutputs.size()
  // + " in-memory map-outputs and " + onDiskMapOutputs.size()
  // + " on-disk map-outputs");
  //
  // // memory percentage to store reduce input.
  // final float maxRedPer = job.getFloat(
  // "mapred.job.reduce.input.buffer.percent", 0f);
  // if (maxRedPer > 1.0 || maxRedPer < 0.0) {
  // throw new IOException("mapred.job.reduce.input.buffer.percent"
  // + maxRedPer);
  // }
  // int maxInMemReduce = (int) Math.min(Runtime.getRuntime().maxMemory()
  ...
  // // write out memory data onto disk to vacate memory.
  // if (numMemDiskSegments > 0) {
  // final Path outputPath = mapOutputFile.getInputFileForWrite(mapId,
  // inMemToDiskBytes).suffix(Task.MERGED_OUTPUT_PREFIX);
  // Writer<K, V> writer = new Writer<K, V>(jobConf, rfs, outputPath, null,
  // null, codec, null);
  //
  // try {
  //
  // ConcatenateMerger.writeFile(writer, memDiskSegments, reporter,
  // spilledRecordsCounter, jobConf);
  ...
  // // 1) from segments in memory
  // long inMemBytes = createInMemorySegments(inMemoryMapOutputs, finalSegments,
  // 0);
  // LOG.info("Merging " + finalSegments.size() + " segments, " + inMemBytes
  // + " bytes from memory into reduce");
  //
  // // 2) from segments on disk
  // long onDiskBytes = 0;
  ...
  // // don't need uncompressed length, these segments used for reading
  // // record by record.
  // // and no need to sort segment by length too.
  // finalSegments.add(new Segment<K, V>(job, fs, file, codec, job
  // .getKeepFailedTaskFiles(), (file.toString().endsWith(
  // Task.MERGED_OUTPUT_PREFIX) ? null : mergedMapOutputsCounter)));
  // }
  // LOG.info("Merging " + onDisk.length + " files, " + onDiskBytes
  // + " bytes from disk");
  // return new UnsortedKVIterator(job, fs, finalSegments, reporter, false)
  // .merge(spilledRecordsCounter, null, null);
  //
  // }
  ...
  private RawKeyValueIterator finalMerge(JobConf job, FileSystem fs,
      List<MapOutput<K, V>> inMemoryMapOutputs, List<Path> onDiskMapOutputs)
      throws IOException {
    LOG.info("finalMerge called with " + inMemoryMapOutputs.size()
        + " in-memory map-outputs and " + onDiskMapOutputs.size()
        + " on-disk map-outputs");
    ...
    if (inMemoryMapOutputs.size() > 0) {
      ...
      if (numMemDiskSegments > 0 && ioSortFactor > onDiskMapOutputs.size()) {
        ...
        // If we reach here, it implies that we have less than
        // io.sort.factor
        // disk segments and this will be incremented by 1 (result of
        // the
        // memory segments merge). Since this total would still be
        // <= io.sort.factor, we will not do any more intermediate
        // merges,
        // the merge of all these disk segments would be directly fed to
        // the
        // reduce method

        mergePhaseFinished = true;
        // must spill to disk, but can't retain in-mem for intermediate
        // merge
        final Path outputPath = mapOutputFile.getInputFileForWrite(mapId,
            inMemToDiskBytes).suffix(Task.MERGED_OUTPUT_PREFIX);
        final RawKeyValueIterator rIter = Merger.merge(job, fs, keyClass,
            valueClass, memDiskSegments, numMemDiskSegments, tmpDir,
            comparator, reporter, false, spilledRecordsCounter, null, mergePhase);
//        final RawKeyValueIterator rIter = Merger.merge(job, fs, keyClass,
//            valueClass, memDiskSegments, numMemDiskSegments, tmpDir,
//            comparator, reporter, spilledRecordsCounter, null, mergePhase);
