...
import org.apache.hadoop.mapred.task.reduce.MapHost.State;
...
class ShuffleScheduler<K, V> {
  ...
  private final int MAX_MAPS_AT_ONCE;
  ...
  ////queue to store penalties host. Penalty Object = (host,delay)
  ...
  public ShuffleScheduler(JobConf job, TaskStatus status,
    ...
    this.MAX_MAPS_AT_ONCE = job.getInt("mapreduce.shuffle.max.maps", 20);
    LOG.info("mapreduce.shuffle.max.maps = " + this.MAX_MAPS_AT_ONCE);
  }

  ////shuffle this mapoutput success.
  ...
  public synchronized void copyFailed(TaskAttemptID mapId, MapHost host,
    ...
    host.penalize();//too many shuffle failure
  ...
  //// map output has been copied and mark it as known.
...
//    host.getBaseUrl() is : http://test07:8099/mapOutput?job=job_201207251327_0001&reduce=0&map=
  ...
  public synchronized List<TaskAttemptID> getMapsForHost(MapHost host) {
    ...
    LOG.info("assigned " + includedMaps + " of " + totalSize + " to " + host
        + " to " + Thread.currentThread().getName());
    return result;
  }
